...
class MaybeLocal {
 public:
  V8_INLINE MaybeLocal() : val_(NULL) {}
  template <class S>
  ...
  V8_WARN_UNUSED_RESULT V8_INLINE bool ToLocal(Local<S>* out) const {
    out->val_ = IsEmpty() ? NULL : this->val_;
    return !IsEmpty();
  }
...
class V8_EXPORT Object : public Value {
 public:
  ...
  bool ForceDelete(Local<Value> key);
   ...
   * change.
   */
  bool HasPrivate(Local<Private> key);
  bool SetPrivate(Local<Private> key, Local<Value> value);
  bool DeletePrivate(Local<Private> key);
  Local<Value> GetPrivate(Local<Private> key);
   ...
   * ObjectTemplate::SetCallAsFunctionHandler method.
   */
  Local<Value> CallAsFunction(Local<Value> recv,
                              int argc,
                              Local<Value> argv[]);
...
class V8_EXPORT ArrayBuffer : public Object {
 ...
 /** PATCH: get data from ArrayBuffer without externalization.
   */
...
class V8_EXPORT ObjectTemplate : public Template {
 public:
  ...
  static Local<ObjectTemplate> New(Isolate* isolate, Local<FunctionTemplate> temp);
